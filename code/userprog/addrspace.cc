...
void AddrSpace::loadPage(int badVAddr)
{
	...
	printf("virtualPage = %i\npageTable[virtualPage].physicalPage = %i\n", virtualPage, pageTable[virtualPage].physicalPage);
	
	pAddr = pageTable[virtualPage].physicalPage * PageSize;
    ...
    if (noffH.code.size > 0)
    {
        file->ReadAt(&(machine->mainMemory[pAddr]), PageSize, noffH.code.inFileAddr);
	}
	
    if (noffH.initData.size > 0)
    {
        file->ReadAt(&(machine->mainMemory[pAddr]), PageSize, noffH.initData.inFileAddr);
    }
